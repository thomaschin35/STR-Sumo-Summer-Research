    ...
    def run(self):
        ...
        try:
            while traci.simulation.getMinExpectedNumber() > 0:
                ...
                vehicle_decisions_by_id, vehicle_states = self.route_controller.make_decisions(vehicles_to_direct, self.connection_info, vehicles_states, step)   
                ...
                if step > MAX_SIMULATION_STEPS:
                    ...
                    break
            if traci.simulation.getMinExpectedNumber() == 0: # this is to get the last vehicle in the time step before closing
                arrived_at_destination = traci.simulation.getArrivedIDList()
                for vehicle_id in arrived_at_destination:
                    if vehicle_id in self.controlled_vehicles:
                        #print the raw result out to the terminal
        ...
        self.connection_info.vehicle_states = vehicle_states
